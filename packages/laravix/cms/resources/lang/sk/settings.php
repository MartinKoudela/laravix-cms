<?php

return [
    'singular' => 'nastavenie',
    'plural' => 'nastavenia',
    'tabs' => [
        'general' => 'Všeobecné',
        'appearance' => 'Vzhľad',
        'seo' => 'SEO',
        'social' => 'Sociálne siete',
        'api' => 'API',
    ],
    'fields' => [
        'site_name' => 'Názov webu',
        'site_description' => 'Popis webu',
        'site_logo' => 'Logo webu',
        'favicon' => 'Favicon',
        'locale' => 'Jazyk',
        'contact_email' => 'Kontaktný e-mail',
        'allow_svg_uploads' => 'Povoliť nahrávanie SVG',
        'meta_title' => 'Meta názov',
        'meta_description' => 'Meta popis',
        'og_image' => 'OG obrázok',
        'google_verification' => 'Google overenie',
        'twitter' => 'X / Twitter',
        'linkedin' => 'LinkedIn',
        'facebook' => 'Facebook',
        'instagram' => 'Instagram',
        'tiktok' => 'TikTok',
        'github' => 'GitHub',
        'youtube' => 'YouTube',
        'discord' => 'Discord',
        'telegram' => 'Telegram',
        'whatsapp' => 'WhatsApp',
        'pinterest' => 'Pinterest',
        'reddit' => 'Reddit',
        'twitch' => 'Twitch',
        'snapchat' => 'Snapchat',
        'spotify' => 'Spotify',
        'robots_txt' => 'robots.txt',
        'api_base_url' => 'Základná URL API',
    ],
    'actions' => [
        'save' => 'Uložiť nastavenia',
        'manage' => 'Správa nastavení',
    ],
    'messages' => [
        'saved' => 'Nastavenia uložené',
    ],
    'hints' => [
        'logo' => 'Zobrazené v hlavičke a päte.',
        'setting_key' => 'Unique setting key, e.g. "site_name", "google_analytics", "logo".',
        'setting_value' => 'The value for this setting.',
        'favicon' => 'Ikona záložky prehliadača. Odporúčané: 32×32 alebo 64×64 PNG.',
        'contact_email' => 'Používa sa v kontaktných formulároch a transakčných e-mailoch.',
        'allow_svg_uploads' => 'SVG súbory môžu obsahovať skripty. Pri nahraní sa čistia, ale nechaj vypnuté, kým SVG logá alebo ikony nepotrebuješ.',
        'meta_title' => 'Predvolený názov stránky, ak nie je nastavený názov na úrovni obsahu.',
        'meta_description' => 'Predvolený meta popis (až 160 znakov).',
        'og_image' => 'Predvolený Open Graph obrázok pre zdieľanie na sociálnych sieťach.',
        'locale' => 'Kód jazyka použitý v <html lang="">. Napr. en, sk, de, fr.',
        'google_verification' => 'Vložte hodnotu atribútu content z meta tagu Google Search Console.',
        'robots_txt' => 'Nechajte prázdne pre predvolené nastavenie (Povoliť všetko). URL sitemapy je vždy pridaná automaticky.',
        'theme' => 'Zmena šablóny sa okamžite premietne do vzhľadu webu pre návštevníkov.',
        'api_base_url' => 'Použite túto URL ako základ pre všetky API požiadavky vo vašom frontende. Každá požiadavka musí obsahovať hlavičku "Authorization: Bearer <token>".',
    ],
    'api_tokens' => [
        'fields' => [
            'name' => 'Názov',
            'token' => 'Token',
            'last_used' => 'Naposledy použitý',
            'expires' => 'Expirácia',
            'created' => 'Vytvorený',
        ],
        'actions' => [
            'create' => 'Vytvoriť token',
            'revoke' => 'Odvolať token',
            'done' => 'Hotovo',
        ],
        'messages' => [
            'reveal_heading' => 'API token vytvorený',
            'reveal_hint' => 'Skopírujte token teraz — už sa znovu nezobrazí.',
            'never_used' => 'Nikdy nepoužitý',
            'never_expires' => 'Nikdy',
            'empty' => 'Zatiaľ žiadne API tokeny',
        ],
    ],
];
